{ config, profiles, ... }:
{
  imports = with profiles; [
    core.rnl
    filesystems.zfs-raid6-dataless
    os.nixos
    type.hypervisor
    backups
  ];
  ...
  rnl.storage = {
    disks = {
      root = [
        "/dev/disk/by-id/ata-WDC_WDS500G1R0A-68A4W0_22291A802536"
        "/dev/disk/by-id/ata-WDC_WDS500G1R0A-68A4W0_23204N400188"
      ];
      # data = [
  ...
  # Networking
  networking = {
    hostId = "768983c1"; # Randomly generated
  ...
  users.users.root.hashedPassword = "$6$GQzOYbDFNZzFjIc5$ft3bnVrf.IjZSLLmSodYBgqhLW6r/jX/.T/1tqFCgqzjPvZFlf4KG6mc3RYMqeoDZQbyaXgKRb2iBPxICJykA1";
