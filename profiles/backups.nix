{
  ...
  systemd.services.syncoid-permissions = let
    datasets = lib.mapAttrsToList (name: {recursive ? false, ...}: (lib.optionalString (!recursive) "-l ") + name) config.services.sanoid.datasets;
    permissions = ["send"]; # TODO: Might need to add permissions
    user = config.services.syncoid.user;

    buildAllowCommand = zfsAction: dataset:
      lib.escapeShellArgs [
