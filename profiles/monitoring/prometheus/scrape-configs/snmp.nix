...
let
  hostsNetworkZeus = lib.rnl.filterHosts [
    (cfg: cfg.rnl.monitoring.snmp)
    (cfg: cfg.rnl.labels.type == "router")
    (cfg: cfg.rnl.labels.os == "junos")
  ] nixosConfigurations;

  hostsNetworkSwitch = lib.rnl.filterHosts [
    (cfg: cfg.rnl.monitoring.snmp)
    (cfg: cfg.rnl.labels.type == "switch")
  ] nixosConfigurations;

  hostsEnergyUPS = lib.rnl.filterHosts [
    (cfg: cfg.rnl.monitoring.snmp)
    (cfg: cfg.rnl.labels.type == "ups")
  ] nixosConfigurations;

  hostsEnergyPDU = lib.rnl.filterHosts [
    (cfg: cfg.rnl.monitoring.snmp)
  ...
  defaultConfig = {
    metrics_path = "/snmp";
    relabel_configs =
      relabelAddressTargetParam ++ relabelInstance ++ relabelInstanceRegex ++ relabelSNMPAddress;
  };
in
