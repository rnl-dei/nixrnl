...
in
{
  options.rnl.nfs = {
    ...
    #TODO: if we decide to use a CephFS subvolume
    # cephHost = mkOption {
    ...
    disk = mkOption {
      type = types.str;
      description = "disk containing the NFS volume";
      default = "/dev/vdb1";
    };
    fsType = mkOption {
      type = types.str;
    ...
    fsOptions = mkOption {
      ...
      default = [ "defaults" ];
      description = "extra options for the disk";
      example = [
        "nofail"
        "noexec"
      ];
  ...
  config = mkIf cfg.enable {
    ...
    services.nfs.server = {
      ...
      exports = ''
        ...
        /mnt/data/cirrus 193.136.164.5/26(rw,async,no_subtree_check,no_root_squash)
      '';
    };
  };
}
