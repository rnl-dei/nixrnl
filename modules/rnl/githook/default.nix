...
in
{
  ...
  config = mkIf cfg.enable {
    services.webhook = {
      ...
      user = cfg.user;
      group = cfg.group;
      hooks = attrsets.mapAttrs' (name: hook: {
    ...
    systemd.tmpfiles.rules = attrsets.mapAttrsToList (
      _name: hook: "d ${hook.path} ${hook.directoryMode} ${hook.directoryUser} ${hook.directoryGroup} - -"
    ) cfg.hooks;
  };
}
