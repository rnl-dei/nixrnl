  ...
  config = mkIf cfg.enable {
    systemd.services.mysql.postStart = lib.mkForce ''
      ...
      ${concatMapStrings (
        ...
        let
          authOption =
            optionalString (user.host == "localhost")
              "IDENTIFIED WITH ${if isMariaDB then "unix_socket" else "auth_socket"}";
        in
        ''
    ...
    environment.etc."mysql.state".text =
      let
        mkDatabase = database: "database:${database}";
        mkUser = user: "user:${user.name}@${user.host}:${mkPermissions user}";
        mkPermissions =
          user: lib.concatStringsSep ";" (mapAttrsToList (n: v: "${n}=${v}") user.ensurePermissions);
      in
      ''
